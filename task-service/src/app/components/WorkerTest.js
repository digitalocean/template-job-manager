...
export default function WorkerTest() {
    ...
    const heartBeat = async (task) => {
            const response = await fetch(`/api/tasks/${task.id}/heartbeat`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    // In a real worker, this comes from HOSTNAME env var.
    ...
    const complete = async (task, taskOutput) => {
    ...
    const doWork = async () => {
        let task = null;
        try {
            // hit /api/tasks/next to get the next task
